...
const Navbar = () => {
  ...
  // ১. লোকাল স্টোরেজ থেকে থিম নেওয়া (Next.js এ window check করা জরুরি)
  ...
  // ২. থিম পরিবর্তন হলে HTML ট্যাগ এবং Local Storage আপডেট করা
  useEffect(() => {
        ...
        <div className="navbar p-0">
          <div className="navbar-start">
            ...
            <div className="dropdown">
              ...
              <ul
                tabIndex={0}
                className="menu menu-sm dropdown-content mt-3 z-[1] p-3 shadow-2xl bg-base-100 rounded-2xl w-52 border border-base-200"
              >
                {navLinks.map((link) => (
                  <li key={link.href}>
                    <Link
            ...
            <ul className="menu menu-horizontal px-1 gap-1 font-semibold">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className={`px-4 py-2 hover:text-primary relative group ${
                      pathname === link.href ? "text-primary" : "text-neutral"
                    }`}
                  >
                    {link.name}
                    <span
                      className={`absolute bottom-1 left-4 h-0.5 bg-primary transition-all duration-300 ${
                        pathname === link.href
                          ? "w-1/2"
          ...
          <div className="navbar-end gap-3">
            {/* --- থিম টগল (আপনার নতুন কোড অনুযায়ী) --- */}
            ...
            {isLoggedIn ? (
              <div className="dropdown dropdown-end">
                ...
                <ul
                  tabIndex={0}
                  className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow-2xl bg-base-100 rounded-box w-52 border border-base-200"
                >
                  <li className="menu-title font-bold text-primary">
                    Dashboard
                  </li>
                  <li>
            ...
            <Link
              href="/book-table"
              className="btn btn-primary rounded-full px-6 shadow-lg hidden sm:flex"
            >
              Book Table
            </Link>
          </div>
