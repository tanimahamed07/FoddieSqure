...
import { Clock, Flame, Utensils, Info, Star, ChevronLeft } from "lucide-react";
import Link from "next/link";
import authOptions from "@/lib/authOptions";
import { getServerSession } from "next-auth";
import { mongoConnect } from "@/lib/mongoConnect";
import MenuActionButtons from "@/component/menu/MenuActionButton";
...
const MenuDetailsPage = async ({ params }: { params: TParams }) => {
  ...
  // সার্ভার সাইড থেকে প্রাথমিক ফেভারিট স্ট্যাটাস চেক
  let initialIsFavorite = false;
  if (session?.user?.email) {
    const { db } = await mongoConnect();
    const existing = await db.collection("favorites").findOne({
      email: session.user.email,
      menuItemId: item._id.toString(), // ID স্ট্রিং হিসেবে চেক করা ভালো
    });
    ...
    <main className="relative min-h-screen bg-base-100 overflow-hidden transition-colors duration-300">
      ...
      <Container>
        <div className="relative z-10 py-10 lg:py-20">
          <Link href="/menu" className="inline-flex items-center gap-2 text-sm font-bold text-primary mb-8 hover:translate-x-[-4px] transition-transform">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
            ...
            <div className="lg:sticky lg:top-24">
              <div className="relative group">
                <div className="relative rounded-[2.5rem] overflow-hidden border-8 border-base-200 shadow-2xl rotate-2 hover:rotate-0 transition-transform duration-500 bg-base-300 aspect-square">
                  <Image src={getSafeImageUrl(item.image)} alt={item.name} fill priority className={`object-cover transition-transform duration-700 group-hover:scale-110 ${!item.isAvailable ? "grayscale" : ""}`} />
                  {!item.isAvailable && (
                    <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                      <span className="bg-error text-white px-6 py-2 rounded-full text-xl font-black uppercase tracking-widest -rotate-12">Sold Out</span>
                    </div>
                  )}
                </div>
                {item.isSpecial && (
              ...
              <div className="mt-8 flex flex-wrap gap-2">
                {item.tags?.map((tag: string) => (
                  <span key={tag} className="bg-base-200 px-4 py-2 rounded-xl text-sm font-bold text-neutral/60 border border-base-300">#{tag}</span>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col">
              <div className="flex justify-between items-center mb-4">
                <span className="bg-primary/10 text-primary px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest border border-primary/20">{item.category}</span>
              ...
              <h1 className="text-4xl lg:text-6xl font-extrabold text-neutral leading-tight mb-6">{item.name} <span className="text-primary italic">.</span></h1>
              <p className="text-lg text-neutral/60 mb-8 leading-relaxed italic border-l-4 border-primary/20 pl-6">"{item.description}"</p>
              ...
              {/* Ingredients */}
              <div className="mb-10">
                <h3 className="text-sm font-black uppercase text-neutral/40 tracking-widest mb-4 flex items-center gap-2"><Utensils size={16} className="text-primary" /> Ingredients</h3>
                <div className="flex flex-wrap gap-2">
                  {item.ingredients?.map((ing: string) => (
                    <span key={ing} className="bg-base-200 px-4 py-2 rounded-2xl text-sm font-semibold border border-base-300 hover:border-primary/50 transition-colors">{ing}</span>
                  ))}
                </div>
              </div>

              {/* Nutrition Card */}
              <div className="bg-neutral text-neutral-content p-6 md:p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden mb-10">
                <h3 className="text-lg font-bold mb-6 flex items-center gap-2"><Flame className="text-orange-500" size={22} /> Nutritional Value</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    ...
                    <div key={i} className="bg-white/5 backdrop-blur-sm p-4 rounded-3xl border border-white/10 text-center">
              ...
              <div className="flex items-center justify-center gap-2 text-xs font-bold opacity-30 mt-6 uppercase tracking-widest">
                <Info size={14} /> <span>Freshly prepared on every order</span>
              </div>
            </div>
          </div>
        </div>
